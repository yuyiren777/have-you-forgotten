...
|------|--------|------|----------|------------|
| **GLM-4.6V-Flash** | 智谱 AI | 免费、适合日程识别 | ✅ 多模态 | OpenAI 兼容 |
...
主力：智谱 GLM-4.6V-Flash（图 → 文字提取 + 日程理解）
  - 性价比最高，中文 OCR 效果最好
  - 约 0.01 元/次
...
```
用户输入 → 有图片？→ 是 → 视觉模型（GLM-4.6V-Flash）
                    → 否 → 智谱 GLM-4.6V-Flash
         → 日程解析（本地引擎）
         → 存入数据库
```
